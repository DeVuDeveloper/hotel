...
module Admin
  module Dashboard
    class RoomsController < ApplicationController
      ...
      before_action :set_hotel, only: %i[index new create edit edit_seasonal_prices update_seasonal_prices]
      before_action :set_room,
        only: %i[edit update destroy edit_seasonal_prices update_seasonal_prices generate_calendars]

      layout "admin"

      def index
        @rooms = @hotel.rooms.ordered if @hotel
        @page_title = "Rooms"
      end
      ...
      def update_seasonal_prices
        if @room.update(seasonal_prices_params)
          respond_to do |format|
            format.html do
              redirect_to admin_dashboard_hotel_rooms_path, notice: "Seasonal prices were successfully updated."
            end
      ...
      def generate_calendars
        ...
        respond_to do |format|
          format.html do
            redirect_to admin_dashboard_hotel_rooms_path, notice: "Calendar entries were successfully updated."
          end
